...
sign_re = re.compile(
	r'(?P<year>\d{4})年(?P<month>\d{1,2})月(?P<day>\d{1,2})日 \(.\) (?P<hour>\d{1,2}):(?P<minute>\d{1,2}) \(UTC\)')
updateddyk_re = re.compile(
	r'\{\{\s*UpdatedDYK\s*\|\s*(.+?)\s*\|\s*(\d+)\s*\}\}', re.IGNORECASE)
updateddyknom_re = re.compile(
	r'\{\{\s*UpdatedDYKNom\s*\|\s*(.+?)\s*\}\}', re.IGNORECASE)
dykinvite_re = re.compile(r'\{\{\s*DYK ?Invite\s*\}\}', re.IGNORECASE)
produce_re = re.compile(
	r'\{\{\s*produceEncouragement\s*\|\s*\d+\s*\}\}', re.IGNORECASE)
...
class DYKEntry(object):
	def __init__(self, content, site, clean=True, quick=False):
		...
		if u'\n}}' in content:
			...
			self.template = NormalizedTemplate(
				...
				[u'article', u'question', u'image', u'type',
				 u'author', u'nominator', u'hash', u'result'],
			)
			if not clean:
				return
			if not quick:
				...
				if article_page:
					...
					if article_page.exists:
						change_template(site, u'Talk:' + article_page.title,
										dykinvite_re, u'{{DYK Invite}}', append=False)
				image = self.template.params[u'image']
				if image:
					image_page = site.getPageForDisplay(u'File:%s' % image)
					if image_page and (
						(site(u'Category:合理使用图像', True)
						 in image_page.referencing(member=True))
							# or
							#	(article_page and (image_page not in article_page.referencing(use=True)))
					):
						self.template.params[u'image'] = u''
			self.template.params[u'question'] = re.sub(
				r'<(s|del|strike)>.*?</\1>', u'', self.template.params[u'question'])
			self.template.params[u'question'] = re.sub(
				r'(\s*[（\(][^（\(）\)]*?[）\)])+$', u'', self.template.params[u'question'])
			self.template.params[u'author'] = re.sub(
				r'<!--.*?-->', u'', self.template.params[u'author'])
			self.template.params[u'nominator'] = re.sub(
	...
	def hash_str(self):  # do not use __hash__
		...
		for k in [u'article', u'question', u'image', u'type', u'author', u'nominator', u'timestamp']:
			sha1.update(hashlib.sha1(
				self.template.params[k].encode('utf8')).digest())
	...
	def check_result(self, site, page, debug):
		...
		if debug:
			print('stage: cr', curdiff, curstage, 'tag', tagdiff, tagstage)
...
def main(debug=False, error_log=None):
	...
	if environ.get('UPDATEDYK_FORCE'):
		...
		try:
			recent_datetime = datetime(
				int(recent_datetime.group('year')), int(
					recent_datetime.group('month')), int(recent_datetime.group('day')),
				int(recent_datetime.group('hor')), int(
					recent_datetime.group('minute')),
			)
		except ValueError:
	...
	if do_update:
		...
		entries.sort(key=lambda entry: not entry.broken and (
			entry.template.params['result'].startswith(
				'*|') or entry.template.params['result'].startswith('!|')
		), reverse=True)
		for entry in entries:
			...
			if result is None:
				...
				if entry.template.params['type'] and (
					entry.template.params['type'] in [
						x['type'] for x in dyk_page.entries[:5]]
				):
					no_type = True
					continue
				...
				if False and entry.template.params['author']:
					match = change_template(
						...
						lambda m: u'{{UpdatedDYK|%s|%d}}' % (
							entry.template.params['article'], int(m.group(2)) + 1),
					...
					if author_dykcount % 5 == 0:
						change_template(
							...
							u'{{produceEncouragement|%d}}' % (
								author_dykcount / 5), None, False,
						)
				# Nominator
			...
			if error_log:
				print('NO_NEW_ENTRY', file=error_log)
				if no_type:
					print('NO_TYPE', file=error_log)
				if no_img:
...
sysop = getSite('zh', 'wikipedia', 'sysop',
				apiErrorAutoRetries=10, httpErrorAutoRetries=50)
produce_re = re.compile(
	r'\{\{\s*produceEncouragement\s*\|\s*(?:count=)?\d+\s*\}\}', re.IGNORECASE)
while True:
	line = raw_input()
	username, count = line.split()
	count = int(count)
	change_template(
		sysop, u'User:%s' % username.decode('utf-8'), produce_re,
		u'{{produceEncouragement|count=%d}}' % count, None, False,
	)
